...
class CollectionViewModel: CollectionViewModelProtocol {
    ...
    func fetchOfData(with text: String, completion: @escaping (Bool) -> (), errorCompletion: @escaping (AFError) -> (), searchButtonPressed: Bool) {
        if searchButtonPressed == true {
            ...
            self.text = text
            networkManager.getArrayOfImages(url: url, searchText: text, page: "1") { [weak self] array in
                if !array.isEmpty {
            ...
            networkManager.getArrayOfImages(url: url, searchText: self.text, page: String(countOfRepeatLoad)) { array in
                if !array.isEmpty {
                    self.arrayOfImages?.append(contentsOf: array)
                    completion(false)
